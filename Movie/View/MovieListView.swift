        ...
        NavigationView {
            VStack {
                TextField("Movie to Search", text: $movieToSearch, onEditingChanged: { _ in }, onCommit: {
                    self.movieListViewModel.movieSearch(movieName: movieToSearch.trimmingCharacters(in: .whitespacesAndNewlines).addingPercentEncoding(withAllowedCharacters: .urlHostAllowed) ?? movieToSearch)
                }).padding()
